...
void OpenGLProgram::CaptureViewPortFloat(cv::Mat& capture, GLenum format, GLuint ch, GLuint framebuf) {
  ...
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      for (int curCh = 0; curCh < ch; ++curCh) {
        tem.at<cv::Vec3b>(i, j)[2-curCh] = (float) 255*(bits[(h - i - 1) * ch * w + j * ch + curCh]);
      }
    }
  }
//  capture = capImg;
...
void OpenGLProgram::CaptureViewPortFloatToFloat(cv::Mat& capture, GLenum format, GLuint ch, GLuint framebuf) {
  ...
  cv::Mat tem = cv::Mat(h,w,CV_32FC3);
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      for (int curCh = 0; curCh < ch; ++curCh) {
        tem.at<cv::Vec3f>(i, j)[2-curCh] = (float) (bits[(h - i - 1) * ch * w + j * ch + curCh]);
      }
    }
  }
//  capture = capImg;
